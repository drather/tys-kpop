...
public class AgencyController {
    ...
    @PutMapping("/{id}")
    public Agency updateAgency(@PathVariable("id") int id, @RequestBody Agency request) {
        Agency agency = agencyService.findById(id);
        agency.setAgency(request.getName(), request.getCeo(), request.getAddress(), request.getImg());
